        ...
        for (int x = 0; x < KX; x = x + h) {
            for (int y = 0; y < KY; y = y + h) {
                while (k < PointList.size()) {
                    H[k][0] = ((x - SatPos[0][k]) / (Math.sqrt(Math.pow(x - SatPos[0][k], 2)) + Math.sqrt(Math.pow(y - SatPos[1][k], 2))));
                    H[k][1] = ((y - SatPos[1][k]) / (Math.sqrt(Math.pow(x - SatPos[0][k], 2)) + Math.sqrt(Math.pow(y - SatPos[1][k], 2))));
                    k += 1;
                }
                k = 0;
